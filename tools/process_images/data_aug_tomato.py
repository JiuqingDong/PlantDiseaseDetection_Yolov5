...
def read_instance_name(root, image_id):
...
if __name__ == "__main__":
    ...
    org_datasets = 'paprika_xmlnoise'
    aug_datasets = 'paprika_xmlnoise10_aug'
    IMG_DIR = "/Volumes/Data/project_paper_code_data/Plant_diseases_dataset/Tomato_dataset/JPEGImages_modify_orientation"
    XML_DIR = "/Volumes/Data/project_paper_code_data/Plant_diseases_dataset/Tomato_dataset/Annotations"
    # 存储增强后的XML文件夹路径
    AUG_XML_DIR = "/Volumes/Data/project_paper_code_data/Plant_diseases_dataset/Tomato_dataset/Annotations_aug"
    try:
    ...
    mkdir(AUG_XML_DIR)
    # 存储增强后的影像文件夹路径
    AUG_IMG_DIR = "/Volumes/Data/project_paper_code_data/Plant_diseases_dataset/Tomato_dataset/JPEGImages_modify_orientation_aug"
    try:
    ...
    seq = iaa.Sequential([
        ...
        iaa.GaussianBlur(sigma=(0, 1)),  # iaa.GaussianBlur(0.5),
        iaa.Affine(
    ...
    for root, sub_folders, files in os.walk(XML_DIR):
        for name in tqdm(files):
            instances = read_instance_name(XML_DIR, name)
            ...
            if ("spot" in instances) or ("death" in instances):
                AUGLOOP = 100
            elif "wilt" in instances:
                AUGLOOP = 20
            elif ("old" in instances) or ("unknown" in instances):
                AUGLOOP = 10
            elif ("canker" in instances) or ("plague" in instances) or ("gmold" in instances) or ("magdef" in instances):
                AUGLOOP = 5
            else:
                AUGLOOP = 0
